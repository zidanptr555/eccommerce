...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_order_id'])){
    ...
    mysqli_query($conn, "DELETE FROM order_items WHERE order_id=$delete_id");

    // Hapus payments terkait (jika ada)
    mysqli_query($conn, "DELETE FROM payments WHERE order_id=$delete_id");
...
<head>
  ...
  <script>
    tailwind.config = {
      theme: {
        extend: { colors: { ugpurple: '#7b2cbf', ugpurplelight: '#9d4edd' } }
      }
    }
  </script>
</head>
...
<?php
// Ambil data order seperti sebelumnya
$search = $_GET['search'] ?? '';
$search_sql = '';
if(!empty($search)){
    ...
    <?php foreach($orders as $order_id => $data): ?>
        <div class="bg-white shadow rounded-xl p-4">
            ...
            <form method="POST" class="mt-3 flex gap-2 items-center">
                ...
                <!-- Tombol Hapus -->
                <button type="submit" name="delete_order_id" value="<?= $order_id; ?>" 
                        class="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600"
                        onclick="return confirm('Yakin ingin menghapus order ini? Semua data terkait akan hilang.');">
                    Hapus
                </button>
            </form>
