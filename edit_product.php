...
if (!$row) {
    echo "<p class='text-red-600 text-center'>Produk tidak ditemukan.</p>";
    exit;
}
...
if (isset($_POST['update'])) {
    ...
    $promo_price = $is_promo ? floatval($_POST['promo_price']) : 0;
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        $query = "UPDATE products 
                  SET name='$name', price=$price, store_area='$store_area', description='$description', 
                      image='$image', is_promo=$is_promo, promo_price=$promo_price
                  WHERE id=$id";
    } else {
        $query = "UPDATE products 
                  SET name='$name', price=$price, store_area='$store_area', description='$description',
                      is_promo=$is_promo, promo_price=$promo_price
                  WHERE id=$id";
    }
...
<form action="" method="POST" enctype="multipart/form-data" class="space-y-4">
    ...
    <label class="font-semibold block mt-2">Deskripsi Produk</label>
    <textarea name="description" rows="4"
              class="w-full border rounded-md px-3 py-2 focus:ring-2 focus:ring-ugpurplelight"><?= htmlspecialchars($row['description']); ?></textarea>

    <img src="assets/img/<?= $row['image']; ?>" class="h-32 rounded-md mb-2">
    ...
    <div class="mt-4 flex items-center gap-2">
        <input type="checkbox" name="is_promo" id="is_promo" value="1" <?= $row['is_promo'] ? 'checked' : '' ?>>
        <label for="is_promo" class="font-semibold">Promo Hari Ini?</label>
    </div>

    <div id="promo_section" class="mt-2" style="<?= $row['is_promo'] ? '' : 'display:none;' ?>">
        <label class="font-semibold block">Harga Promo</label>
        <input type="number" name="promo_price" value="<?= $row['promo_price'] ?>" 
               class="w-full border rounded-md px-3 py-2 focus:ring-2 focus:ring-ugpurplelight">
    </div>
...
isPromoCheckbox.addEventListener('change', function() {
    promoSection.style.display = this.checked ? 'block' : 'none';
});
</script>
