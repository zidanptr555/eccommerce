...
foreach ($_SESSION['cart'] as $product_id => $qty) {
    $q = mysqli_query($conn, "SELECT price, store_area FROM products WHERE id = $product_id");
    $p = mysqli_fetch_assoc($q);

    $totalBelanja += $p['price'] * $qty;

    // Ambil store_area dari produk pertama di keranjang
...
<div class="max-w-4xl mx-auto mt-8 bg-white shadow-xl p-6 rounded-2xl">
    ...
    <form action="checkout_process.php" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input required name="nama_penerima" type="text"
        ...
        <div>
            ...
            <input required name="hp_penerima" type="text"
                   class="w-full p-2 border rounded-md mt-1">
        </div>
        ...
        <div>
            ...
            <textarea required name="alamat_penerima"
                      class="w-full p-3 border rounded-md h-24 mt-1"></textarea>
        </div>

        <!-- Asal Toko -->
        <div>
            <label class="font-semibold">Asal Daerah Toko</label>
            <input id="asal_toko" value="<?= $asal_toko ?>" readonly
                   class="w-full p-2 border bg-gray-100 rounded-md mt-1 font-bold">
        </div>

        <!-- Tujuan Pengiriman -->
        <div>
            <label class="font-semibold flex items-center gap-2">
                <span class="text-2xl">🚚</span> Tujuan Pengiriman
            </label>
        ...
        <p class="text-xl font-bold text-ugpurple">
            Total Akhir:
            <span id="total_akhir">Rp <?= number_format($totalBelanja, 0, ',', '.') ?></span>
        </p>

        <button class="w-full bg-green-600 text-white py-3 rounded-lg font-bold hover:bg-green-700 mt-4"
                type="submit">
            Bayar Sekarang
        </button>
